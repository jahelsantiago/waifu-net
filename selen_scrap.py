...
def pages_loop(waifu_type, n_images, width, height, path, name_to_save):
    ...
    def __scrape(driver, n_images):
        for i in range(1, n_images + 1):
            try:
                if i % 3 == 0:
                    driver.execute_script("window.scrollTo(0,document.body.scrollHeight)")

                content_a = driver.find_element_by_xpath("""/html/body/div[2]/c-wiz/div[3]/div[1]/div/div/div/div/div[1]/div[1]/div[{0}]/a/div/img""".format(i)).click()
                time.sleep(0.5)
    ...
    def __save_images(path, name_to_save):
        counter = 0
    ...
    __scrape(driver, n_images)
    __save_images(path, name_to_save)
    driver.close()
